...
DASHBOARD_STORAGE_ACC_NAME="${APP_PREFIX}dashboard"
...
echo "Creating storage account for static page content with name $DASHBOARD_STORAGE_ACC_NAME"
az storage account create -n $DASHBOARD_STORAGE_ACC_NAME -g $RG_NAME -l $RG_LOC --sku Standard_LRS --kind "StorageV2" --output none
STORAGE_ACC_CONNECTION_STR=$(az storage account show-connection-string -g $RG_NAME -n $DASHBOARD_STORAGE_ACC_NAME)
echo "done"

#enabling static website on blob storage
echo "Enabling static page mode on storage account $DASHBOARD_STORAGE_ACC_NAME"
az storage blob service-properties update --account-name $DASHBOARD_STORAGE_ACC_NAME --static-website --index-document index.html --output none
DASHBOARD_URL=$(az storage account show --name $DASHBOARD_STORAGE_ACC_NAME --query 'primaryEndpoints.web' | tr -d '"' | awk -F[/:] '{print $4}')
echo "done"
...
az storage account create -n $FUNC_WEBSOCKET_STORAGE_NAME -g $RG_NAME -l $RG_LOC --sku Standard_LRS --kind "StorageV2" --functions-version 2 --output none
...
az storage account create -n $FUNC_DEVICE_SERVICES_STORAGE_NAME -g $RG_NAME -l $RG_LOC --sku Standard_LRS --kind "StorageV2" --functions-version 2 --output none
